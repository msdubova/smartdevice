...
function controlModal() {
  ...
  var closeButton = modalBlock.querySelector('.modal__button--close');
  var body = document.querySelector('.page');
  var overlay = document.querySelector('.modal__overlay');
  var nameInput = document.querySelector('#modal-name'); // Функция отслеживает клавишу ESCape в режиме открытого модально окна, и закроет его в случае нажатия отслеживаемой клавиши
...
controlModal(); // Функция создает маску на номер телефона в элементе input tel
...
maskPhone(); // Функция которая отслеживает размер вьюпорта чтоб запустить функции, актуальные для разных вьюпортов
...
function setAccordeon() {
  ...
  var accordeons = document.querySelectorAll('.accordeon__block');
  var pluses = document.querySelectorAll('.footer__toggle');
  ...
  var _loop = function _loop(_i) {
    buttons[_i].addEventListener('click', function () {
      for (var y = 0; y < accordeons.length; y++) {
        if (y !== _i) {
      ...
      accordeons[_i].classList.toggle('accordeon__block--closed');

      pluses[_i].classList.toggle('footer__toggle--plus');

      pluses[_i].classList.toggle('footer__toggle--minus');
    });
  };
...
setResize(setAccordeon); // Функция открытия закрытия аккордеона по типу "Подробнее" в разделе Aboutus

function setTextAccordeon() {
  var button = document.querySelector('.aboutus__button');
  var hiddenElements = document.querySelectorAll('.aboutus__hidden');
  var hiddenElementDesktop = document.querySelector('.aboutus__hidden--desktop');
  var hiddenElementMobile = document.querySelector('.aboutus__hidden--mobile');

  for (var y = 0; y < hiddenElements.length; y++) {
    if (window.innerWidth < 768) {
    ...
    window.addEventListener('resize', function () {
      if (window.innerWidth < 768) {
        if (button.innerHTML === 'Подробнее') {
          ...
          for (var _i2 = 0; _i2 < hiddenElements.length; _i2++) {
            hiddenElements[_i2].classList.remove('visually-hidden');
          }
        }
      } else {
